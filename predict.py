...
def predict_no_ui(inputs, top_p, temperature, history=[], sys_prompt="", user=''):
    """
    ...
    if all_user[current_user]['counts'] <= 0:
        chatbot.append((inputs, "余额不足"))
        yield chatbot, history, "余额不足"
        return
    retry = 0
    while True:
        try:
    ...
    all_user[user]['counts'] -= num_tokens_from_messages(payload["messages"])
...
def predict(inputs, top_p, temperature, chatbot=[], history=[], system_prompt='', login=False, current_user=None,
    ...
    if not login:
        chatbot.append((inputs, "请先在右侧登陆"))
        yield chatbot, history, "未登陆"
        return
    all_user = global_var.get_value('all_user')
        ...
        while True:
            ...
            if chunk:
                try:
                    ...
                    all_user[current_user]['counts'] -= 1
                    ...
                    return
        all_user[current_user]['counts'] -= num_tokens_from_messages(payload["messages"])
        # global_var.set_value('all_user', all_user)

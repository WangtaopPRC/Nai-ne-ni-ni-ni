...
def 解析Paper(file_manifest, project_folder, top_p, temperature, chatbot, history, systemPromptTxt, user):
    import time, os
...
@CatchException
def 读文章写摘要(txt, top_p, temperature, chatbot, history, systemPromptTxt, flag, current_user, WEB_PORT):
    if not flag:
        return
    history = []  # 清空历史，以免输入溢出
    import glob, os

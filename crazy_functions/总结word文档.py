...
def 解析docx(file_manifest, project_folder, top_p, temperature, chatbot, history, systemPromptTxt, current_user):
    import time, os
    ...
    for index, fp in enumerate(file_manifest):
        ...
        if not fast_debug:
            ...
            gpt_say = yield from predict_no_ui_but_counting_down(i_say, i_say_show_user, chatbot, top_p, temperature,
                                                                 history=[], user=current_user)  # 带超时倒计时
    ...
    if not fast_debug:
        ...
        gpt_say = yield from predict_no_ui_but_counting_down(i_say, i_say, chatbot, top_p, temperature,
                                                             history=history, user=current_user)  # 带超时倒计时
...
@CatchException
def 总结word文档(txt, top_p, temperature, chatbot, history, systemPromptTxt, flag, current_user, WEB_PORT):
    import glob, os
    if not flag:
        return
    ...
    # 开始正式执行任务
    yield from 解析docx(file_manifest, project_folder, top_p, temperature, chatbot, history, systemPromptTxt, current_user)
